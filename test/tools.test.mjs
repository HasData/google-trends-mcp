// Tool contract test.
//
// The README promises one tool with a specific name and required parameter. The upstream list
...
const ENDPOINT = 'https://mcp.hasdata.com/api/mcp?apis=google_trends';
const KEY = process.env.HASDATA_API_KEY;
const TIMEOUT_MS = 30_000;

const EXPECTED = {
    hasdata_google_trends_search_getTrendsData: 'q',
};
...
test('apis=google_trends exposes exactly one tool', live, async () => {
...
test('the key is accepted by HasData', live, async () => {
    const { raw } = await rpc('tools/call', {
        name: 'hasdata_google_trends_search_getTrendsData',
        arguments: { q: 'coffee' },
    });
